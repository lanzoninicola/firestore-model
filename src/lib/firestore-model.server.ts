...
export default class FirestoreModel<T> {
    constructor(private _client: FirestoreClient, private _collectionName: string) {}
    ...
    async findAll(): Promise<T[]> {
        ...
        const querySnapshot = await getDocs(collection(this._client.connection, this._collectionName));
    ...
    async findById(documentId: string): Promise<T | FirestoreDocumentNotFound> {
        const docRef = doc(this._client.connection, this._collectionName, documentId);
    ...
    async add(data: { [key: string]: any }): Promise<T> {
        try {
            const docRef = await addDoc(collection(this._client.connection, this._collectionName), {
                ...data,
     ...
     * - *Error response*: {ok: false, error: any}
     */
    async update(documentId: string, updatedData: any): Promise<true | FirestoreDocumentUpdateError> {
        try {
            const docRef = doc(this._client.connection, this._collectionName, documentId);
     ...
     * - *Error response*: {ok: false, error: any}
     */
    async delete(documentId: string): Promise<true | FirestoreDocumentDeletionError> {
        try {
            await deleteDoc(doc(this._client.connection, this._collectionName, documentId));
            return true;
        } catch (e) {
            if (e instanceof FirebaseError) {
                return new FirestoreDocumentCreationError(e.message);
    ...
    async deleteAll(): Promise<true | FirestoreDocumentDeletionError> {
        try {
            ...
            querySnapshot.forEach(async (document) => {
                await deleteDoc(doc(this._client.connection, this._collectionName, document.id));
            });
    ...
    async findWhere(field: string, operator: WhereFilterOp, value: any): Promise<T[]> {
        ...
        const querySnapshot = await getDocs(
            query(collection(this._client.connection, this._collectionName), where(field, operator, value))
        );
    ...
    async findWhereIn(field: string, values: any[]): Promise<T[] | FirestoreDocumentNotFound> {
        let result: FirestoreDocument[] = [];

        const querySnapshot = await getDocs(
            query(collection(this._client.connection, this._collectionName), where(field, "in", values))
        );
